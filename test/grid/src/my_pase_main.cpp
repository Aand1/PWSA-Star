...
//    debug = pasl::util::cmdline::parse_or_default_bool("debug", false);
  ...
  auto run = [&] (bool sequential) {
    auto heuristic = [&] (int u, int v) {
      return G.weighted_euclidean(w, u, v);
    };

    res = pase<Graph, decltype(heuristic)>(G, heuristic, src, dst, exptime, pebbles);
  };
  ...
  auto output = [&] {
    ...
    // }
    for (int i = 0; i < G.number_vertices(); i++) {
      if (res[i].is_expanded) num_expanded++;
