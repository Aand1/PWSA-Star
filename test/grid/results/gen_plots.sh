...
./scripts/reformat.py -keep "{'exptime':['0.0'],'w':['1.0'],'map':['maps/sc1/Expedition.map.big-4','maps/bgmaps/AR0602SR.map.big-8']}" -exclude "{'algo':['simple_wPWSA*']}" -input $RESULTS -output results/temp.txt
pplot scatter -x proc -y exectime -series algo -chart map -input results/temp.txt -output results/exptime0_w1_exectime.pdf

# plot exptime=0, w=1.0,1.1,1.5,2.0: wA*, wPWSA*
./scripts/reformat.py -keep "{'algo':['wA*','wPWSA*'],'exptime':['0.0'],'w':['1.0','1.1','1.5','2.0'],'map':['maps/sc1/Expedition.map.big-4','maps/bgmaps/AR0602SR.map.big-8']}" -input $RESULTS -output results/temp.txt
...
./scripts/reformat.py -keep "{'algo':['wA*','wPWSA*','wPA*SE','wPA*NRE'],'exptime':['0.000005','0.00001','0.00003','0.00005'],'w':['1.0'],'map':['maps/sc1/Expedition.map.min-2','maps/bgmaps/AR0602SR.map']}" -input $RESULTS -output results/temp.txt
