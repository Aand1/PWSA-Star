...
#define BUCKETS 256    // 1 << MAX_RADIX
  ...
  /*
   template <class uintT, class intT>
   ...
   template <class T, class uintT, class intT>
   void integerSort(std::pair<uintT,T> *A, intT n) {
   ...
   template <class T, class uintT, class intT>
   void integerSort(std::pair<uintT,T> *A, intT n, char* s) {
   intT maxV = sequence::mapReduce<uintT>(A,n,utils::maxF<uintT>(),
   utils::firstF<uintT,T>());
