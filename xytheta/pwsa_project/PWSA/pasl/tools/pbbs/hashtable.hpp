...
namespace pbbs {
  ...
  private:
    ...
    uintT hashToRange(intT h) {return uintT(h & mask);}
  ...
  public:
    ...
    _seq<eType> entries() {
      bool *FL = newA(bool,m);
      native::parallel_for(intT(0), m, [&] (intT i) { FL[i] = (TA[i] != empty); });
  ...
  _seq<ET> removeDuplicates(_seq<ET> S, intT m, HASH hashF) {
    ...
    ET* A = S.A;
    {native::parallel_for(intT(0), S.n, [&] (intT i) { T.insert(A[i]); });}
    _seq<ET> R = T.entries();
    T.del();
    return R;
  }
  ...
  template <class Edge>
  struct hashEdge {
    typedef typename Edge::vtxid_type intT;
    typedef Edge* eType;
    typedef Edge* kType;
    eType empty() {return NULL;}
    kType getKey(eType v) {return v;}
    intT hash(kType e) {
      return utils::hashInt(e->src) + utils::hashInt(100*e->dst); }
    int cmp(kType a, kType b) {
      int c = cmpInt(a->src, b->dst);
      return (c == 0) ? cmpInt(a->src,b->dst) : c;
    }
    bool replaceQ(eType v, eType b) {return 0;}
  };
  ...
  _seq<Edge* > removeDuplicates(_seq<Edge *>  S) {
    return removeDuplicates(S,hashEdge<Edge>());}
  
} // end namespace

#endif /*! _PBBS_HASHTABLE_H_ */