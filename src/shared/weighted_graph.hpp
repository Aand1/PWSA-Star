...
public:
    ...
    for (int v = 0; v < vertices.size(); v++) {
      ...
      for (int i = 0; i < 8; i++) {
        ...
        if (0 <= nghrow && nghrow < height &&
          ...
          neighbor u;
          u.vertex = grid[nghrow][nghcol];
          u.weight = nghrow * dist[i]; // life cost multiplies by position in the grid
          neighbors.push_back(u);
          curr_offset++;
        }
      }
    ...
    for (int v = 0; v < vertices.size(); v++) {
      ...
      for (int i = 0; i < 8; i++) {
        ...
        if (0 <= nghrow && nghrow < height &&
            ...
            grid[nghrow][nghcol] != OBSTACLE) {
          if (neighbors[neighbor_offsets[v] + deg].vertex != grid[nghrow][nghcol]) return false;
          if (neighbors[neighbor_offsets[v] + deg].weight != nghrow * dist[i]) return false;
          deg++;
        }
      }
      if (deg != degree(v)) return false;
  ...
  void pebble_dump(SearchResultT<long>* result, int src, int dst, const char* fname) {
    int* pebbles = new int[vertices.size()];
    int* predecessors = new int[vertices.size()];
    for (int i = 0; i < vertices.size(); i++) {
      pebbles[i] = result->pebble(i);
  ...
  // weight of arc (u,v)
  long weight_between(int u, int v) {
    int u_row = vertices[u].row;
    int u_col = vertices[u].col;
    ...
    int diff = abs(u_row - v_row) + abs(u_col - v_col);
    if (diff == 0) return 0;
    else if (diff == 1) return u_row * 10000L;
    else if (diff == 2) return u_row * 14142L;
    else {
      std::cerr << "ERROR (weight_between): vertices (" << u << "," << v << ") not adjacent" << std::endl;
      std::exit(EXIT_FAILURE);
    }
  ...
  long pathlen(SearchResultT<long>* res, int src, int dst) {
    long len = 0;
    for (int curr = dst; curr != src; curr = res->predecessor(curr)) {
//      std::cout << curr << " " << std::flush;
      len += weight_between(res->predecessor(curr), curr);
    }
//    std::cout << std::endl;
    return len;
  }

  // taken from PBNF source, author = Ethan Burns 2009
  int cost_from (int a, int b) const {
  	int last = b;
  ...
  // adapted from PBNF source, author = Ethan Burns 2009
  long weighted_manhattan (double w, int u, int v) const {
  	int x = vertices[u].col;
  	int y = vertices[u].row;
  	int gx = vertices[u].col;
  	int gy = vertices[u].row;
  	int dx = abs(gx - x);
  	int dy = abs(gy - y);
  	...
  	if (dx > dy) {
			...
			return (cost_from(y, high_y)
				+ across * high_y
				+ cost_from(high_y, gy)) * (long) (10000.0 * w);
		} else {
			return (cost_from(y, gy) * (long) (10000.0 * w));
		}
  }
