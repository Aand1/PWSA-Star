...
template <class GRAPH, class HEURISTIC>
std::pair<std::atomic<vertpack>*, std::atomic<bool>*>
pwsa_new(GRAPH& graph, const HEURISTIC& heuristic,
         const int& source, const int& destination,
         int split_cutoff, int poll_cutoff, double exptime,
         int* pebbles = nullptr) {

  using Frontier = Heap<Range>;
  ...
  pasl::sched::native::parallel_for(0, N, [&] (int i) {
    vertpack ith;
    ith.distance = INFINITE_DISTANCE;
    ith.pred = INFINITE_DISTANCE;
    gpred[i].store(ith);
  });

  vertpack src_pack;
  src_pack.distance = 0;
  src_pack.pred = source;
  gpred[source].store(src_pack);

  Range src_range;
  src_range.edge_start = 0;
  src_range.edge_end = graph.degree(source);

  Frontier initF = Frontier();
  initF.insert(heuristic(source), src_range);
  ...
  auto do_work = [&] (bool& is_done, Frontier& frontier) {
    int work_this_round = 0;
    while (work_this_round < poll_cutoff && frontier.size() > 0 && !is_done) {

      Range* top = frontier.peek();
      Range curr;
      if (top->edge_end - top->edge_start < poll_cutoff - work_this_round) {
        curr = *top;
        frontier.discard_min();
      }
      else {
        curr.vertex = top->vertex;
        curr.edge_start = top->edge_start;
        curr.edge_end = top->edge_start + poll_cutoff - work_this_round;
        top->edge_start = curr.edge_end;
      }

      graph.for_each_neighbor_in_range(curr.vertex, curr.edge_start, curr.edge_end, [&] (int v, int weight) {

        vertpack gpred_curr = gpred[curr.vertex].load();
        vertpack my_v;
        my_v.distance = gpred_curr.distance + weight;
        my_v.pred = curr.vertex;

        // attempt to decrease the distance to v using write-with-min
        do {
          gpred_v = gpred[v].load();
        } while (my_v.distance < gpred_v.distance
                 && !gpred[v].compare_exchange_weak(gpred_v, my_v));

        bool orig = is_expanded[v].load();
        if (!orig && is_expanded[v].compare_exchange_strong(orig, true)) {
          // We claim v for expansion. Simulate "get successors" step by busy
          // looping for (max-out-degree * exptime) secs
          graph.simulate_get_successors(exptime);

          Range v_range;
          v_range.vertex = v;
          v_range.edge_start = 0;
          v_range.edge_end = graph.degree(v);

          frontier.insert(gpred[v].load().distance + heuristic(v), v_range);
        }
      });

      work_this_round += curr.edge_end - curr.edge_start;
    }
    work_since_split.mine() += work_this_round;
  };
