...
_body						\
    ...
    template <class OT, class intT, class G>
    OT plusReduce(OT* A, intT n, G g) {
      return reduce<OT>((intT)0,n,utils::addF<OT>(), g);
    }
    
    template <class OT, class intT>
    OT plusReduce(OT* A, intT n) {
      return plusReduce<OT>((intT)0,n,getA<OT,intT>(A));
    }
    
    template <class OT, class intT, class G>
    OT maxReduce(OT* A, intT n, G g) {
      return reduce<OT>((intT)0,n,utils::maxF<OT>(), g);
    }
    
    template <class OT, class intT>
    OT maxReduce(OT* A, intT n) {
      return maxReduce(A, n, getA<OT,intT>(A));
    }
